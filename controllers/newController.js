...
exports.updateNew = catchAsync(async (req, res, next) => {
  ...
  if (
    req.body.thumbnail &&
    doc.thumbnail &&
    req.body.thumbnail !== doc.thumbnail
  ) {
    fs.unlink(doc.thumbnail, (err) => {
      if (err) console.error(`Error removing old thumbnail: ${err}`);
    });
  }
  ...
  if (req.body.images && Array.isArray(req.body.images)) {
    const imagesToDelete = doc.images.filter(
      (oldImage) => !req.body.images.includes(oldImage)
    );
  ...
  // Filter the body to only allow specific fields
  ...
  // Update the document
  const updatedDoc = await New.findOneAndUpdate(
