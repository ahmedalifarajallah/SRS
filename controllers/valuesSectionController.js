...
exports.getValuesSections = catchAsync(async (req, res, next) => {
    ...
    const data = await ValuesSection.find().sort('-createdAt');
...
exports.updateValuesSection=catchAsync(async(req,res,next)=>{
 ...
 if(req.files.main&&req.files.rotate){ fs.unlink(`${req.valuesSection.images.rotate}`, (err) => {}); fs.unlink(`${req.valuesSection.images.main}`, (err) => {});}
...
exports.checkValuesSectionExists = async (req, res, next) => {
  ...
  exports.constraint=catchAsync(async(req,res,next)=>{
        ...
        if(data.length>=3) return next(new AppError(`You reach to values limit and cannot add anymore`,401));
        next();
  })
